...
export default function WordToPdf({ endpoint }) {
    ...
    const handleFile = (f) => {
        ...
        if (!f.name.match(/\.(docx?)$/i)) {
            setError('Veuillez sélectionner un fichier Word (.doc ou .docx)');
            return;
        }
        setError(null);
        setPdf(null);
    ...
    return (
        <div className="max-w-5xl mx-auto px-6 pt-14 pb-24">
            <div className="text-center mb-12 fade-up">
                <h1 className="text-white text-4xl font-bold mb-3">Word en PDF</h1>
            ...
            <div className="max-w-2xl mx-auto mb-8 fade-up fade-up-delay-1">
                <div className="bg-white/[0.04] backdrop-blur-xl rounded-2xl border border-white/[0.08] p-6">
                    <form onSubmit={handleSubmit}>
                        <div
                            className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition mb-4 ${dragging ? 'border-violet-larry bg-violet-larry/10' : file ? 'border-violet-larry/40 bg-violet-larry/5' : 'border-white/10 hover:border-white/20'}`}
                        ...
                        >
                            {file ? (
                                <>
                                    <i className="fa-solid fa-file-word text-3xl text-blue-400 mb-3 block"></i>
                                ...
                                <>
                                    ...
                                    <p className="text-white/50 text-sm">Glissez votre fichier .docx ici ou <span className="text-violet-larry">cliquez pour choisir</span></p>
                                </>
                            )}
                            <input ref={inputRef} type="file" accept=".doc,.docx" className="hidden" onChange={e => handleFile(e.target.files[0])} />
                        </div>
                ...
                <div className="max-w-4xl mx-auto fade-up">
                    <div className="bg-white/[0.04] backdrop-blur-xl rounded-2xl border border-white/[0.08] overflow-hidden">
                        <div className="h-[200px] flex flex-col items-center justify-center gap-4 text-white/20">
                            <i className="fa-solid fa-file-word text-5xl text-blue-400/30 animate-pulse"></i>
